...
临床标准，比方说邮件系统有各种服务端、客户端应用，形态、功能差异巨大，有的集成了联系人方便发送，有的集成了日程管理，这样的例子SMTP，大家遵循的mail协议，每个应用有不同的形态，不同的功能，更恰当的例子还有浏览器应用，他是随着HTML的发展，是新技术的缩影，快速变化的缩影，竞争的缩影。大家可以共用共享共存。

标准化应用
系统标准，大家诟病粒度太粗，不易平衡，认证复杂，流于形式（）。指导方案又很容易有厂家倾向性，具体到业务选型本是一个推荐。反而成为控标参数。

自由or约束
从辩证法角度答案当然是辨证统一的。针对开发平台同样是如此，现阶段的luna将约束可控，作为第一要素。
## 平台愿景
